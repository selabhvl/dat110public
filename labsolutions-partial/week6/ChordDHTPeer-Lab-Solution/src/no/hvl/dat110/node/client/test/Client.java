...
public class Client {
	...
	public static List<BigInteger> getKeys(String[] files){
		
		List<BigInteger> replicas = new ArrayList<>();
		for(String file : files) {
			BigInteger[] reps = FileManager.createReplicaFiles(file, Util.numReplicas);
	...
	public static void main(String[] args) throws NoSuchAlgorithmException, IOException {
		...
		String[] files = {"file1.txt"}; 	// we just limit to 5 files ,"file5.txt"
		...
		List<String> peers = findPeersofFile(file);
		System.out.println(peers);
		
		List<BigInteger> keys = getKeys(files);
